...
<div class="page-heading">
    ...
    <section class="section">
        <div class="card">
            <div class="card-body">
                <form method="post" action="{{ route('hrm.payroll.update', $payroll) }}">
                    @csrf
                    @method('patch')
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="employee_id" class="form-select">
                                @foreach($employees as $e)
                                    <option value="{{ $e->id }}" {{ old('employee_id', $payroll->employee_id) == $e->id ? 'selected' : '' }}>{{ $e->first_name }} {{ $e->last_name }} ({{ $e->email }})</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <input type="number" name="year" class="form-control" value="{{ old('year', $payroll->year) }}" min="2000" max="2100">
                            @error('year')<small class="text-danger">{{ $message }}</small>@enderror
                        </div>
                        <div class="col-md-3 mb-3">
                            <label class="form-label">Month</label>
                            <input type="number" name="month" class="form-control" value="{{ old('month', $payroll->month) }}" min="1" max="12">
                            @error('month')<small class="text-danger">{{ $message }}</small>@enderror
                        </div>
                        <div class="col-md-4 mb-3">
                            <label class="form-label">Basic Salary</label>
                            <input type="number" step="0.01" name="basic_salary" class="form-control" value="{{ old('basic_salary', $payroll->basic_salary) }}">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="status" class="form-select">
                                @foreach(['draft','approved','paid'] as $s)
                                    <option value="{{ $s }}" {{ old('status', $payroll->status) === $s ? 'selected' : '' }}>{{ ucfirst($s) }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>

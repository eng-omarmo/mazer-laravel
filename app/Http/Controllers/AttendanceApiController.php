...
class AttendanceApiController extends Controller
{
    public function sync(Request $request)
    {
        ...
        $status = 'present';
        if (empty($validated['check_in'])) $status = 'absent';
        else {
            if (strtotime($validated['check_in']) > strtotime('09:15')) $status = 'late';
            if (!empty($validated['check_out']) && strtotime($validated['check_out']) < strtotime('17:00')) $status = 'early_leave';
        }
